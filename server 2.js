...
app.get('/pieChart', (req, res) => {
    var sqlFood = "SELECT SUM(price) FROM `'1595123453411'` WHERE category = 1";
    var sqlTrans = "SELECT SUM(price) FROM `'1595123453411'` WHERE category = 2";
    var sqlSavings = "SELECT SUM(price) FROM `'1595123453411'` WHERE category = 3";
    var sqlOther = "SELECT SUM(price) FROM `'1595123453411'` WHERE category = 4";
...
app.get('/monthlyIncomeTable', (req, res) => {
    var sql = "SELECT * FROM `'1595123453411'`";
    con.query(sql, function (err, results) {
        if (err) throw err;
...
app.post('/login', passport.authenticate('local', { failureRedirect: '/login', failureFlash: true }), function (req, res) {
    res.redirect('/monthlyIncomeTable')
})
...
app.post('/adding', (req, res) => {
    ...
    con.query('INSERT INTO `?` (category, price, description, year, month, date) VALUES ?', [id, values], function (err, result) {
        if (err) throw err;
    })
    res.redirect('/adding')
});
